...
internal partial class UserService
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        return result.Succeeded
            ? string.Format(
                _t["Account Confirmed for E-Mail {0}. You can now use the /api/tokens endpoint to generate JWT."],
                user.Email)
            : throw new InternalServerException(string.Format(_t["An error occurred while confirming {0}"],
                user.Email));
    }
    ...
    public async Task<string> ConfirmPhoneNumberAsync(string userId, string code)
    {
        ...
        if (string.IsNullOrEmpty(user.PhoneNumber))
            throw new InternalServerException(_t["An error occurred while confirming Mobile Phone."]);
